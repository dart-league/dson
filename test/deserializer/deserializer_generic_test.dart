...
@serializable
class GenericTClass<T> extends SerializableMap with _$GenericTClassSerializable<T> {
  T? t;
}

@serializable
class GenericT1T2Class<T1, T2> extends SerializableMap with _$GenericT1T2ClassSerializable<T1, T2> {
  T1? t1;
...
main() {
  ...
  test('deserialize: generic List<T>', () {
    ListTClass<SimpleClass> test = fromJson('{"listT": [{"name": "test1"}, {"name": "test2"}]}',
            [() => ListTClass<SimpleClass>(), {'listT': [() => List<SimpleClass>.empty(growable: true), SimpleClass]}]);
  ...
  test('deserialize: GenericT1T2Class', () {
    GenericT1T2Class<SimpleClass, List<SimpleClass>> test = fromJson('{"t1": {"name": "test"}, "t2": [{"name": "test"}]}',
        [() => GenericT1T2Class<SimpleClass, List<SimpleClass>>(), {'t1': SimpleClass, 't2': [() => List<SimpleClass>.empty(growable: true), SimpleClass]}]);
