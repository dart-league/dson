...
//  _serLog.fine("Start serializing");
  ...
  var result =
      JSON.encode(objectToSerializable(object, depth: depth, exclude: exclude));
...
List _serializeList(List list, depth, exclude, String fieldName) {
  ...
  list.forEach((item) {
    newList.add(objectToSerializable(item,
        depth: depth, exclude: exclude, fieldName: fieldName));
  });
...
//    return {'index': obj.index, 'name': obj.toString().split(".")[1]};
  ...
  if (classMirror?.annotations?.any((x) => x is OnSerialize) == true) {
    obj[((classMirror.annotations.firstWhere((x) => x is OnSerialize))
            as OnSerialize)
        .toRun]();
  }
    ...
    if (depth != null || !_isCiclical(classMirror) || fieldName == null) {
      publicVariables.forEach((fieldName, decl) {
        if (!fieldName.startsWith('_'))
          _pushField(fieldName, decl, obj, result, depth, exclude);
      });
...
void _pushField(String fieldName, DeclarationMirror variable,
    SerializableMap obj, Map<String, dynamic> result, depth, exclude) {
...
_getNext(excludeOrDepth, String fieldName) {
  if (excludeOrDepth is List) {
    excludeOrDepth = excludeOrDepth.firstWhere(
        (e) => //
  ...
  if (excludeOrDepth is String && excludeOrDepth == fieldName)
    return excludeOrDepth;
}
